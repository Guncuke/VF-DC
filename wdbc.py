...
# X = torch.load('localdp_wdbc.pt')['data']
# X_train = X[0][0].squeeze().to(device)
# y_train = X[0][1].to(device)
...
class SimpleNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc2(x)
        x = self.relu(x)
        x = x + self.noise.detach()
        x = self.fc3(x)
        return x

input_size = 30

style, channel, im_size, num_classes, class_names, mean, std, dst_train, dst_test, testloader = get_dataset('WDBC', 'data')
...
torch.save([mean, std], 'whole_wdbc.pt')
